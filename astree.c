...
PROCFUNCDEF *make_procfuncdef (DECLLIST *fo, IDENT *t, BLOCK *b)
{
    PROCFUNCDEF *p = anew (PROCFUNCDEF);
    p->proc_formals = fo;
    p->proc_result_type = (t) ? make_id_type(t) : NULL;
    p->proc_block = b;
...
DECL *make_procfunc_decl (IDENT *id, DECLLIST *fo, IDENT *t, BLOCK *b)
{
    DECL *d = make_decl(DeclProcFunc_, id);
    d->d.proc = make_procfuncdef(fo, t, b);
    return d;
}
...
TYPE *make_arr_type(TYPE *idx, TYPE *elt) 
{
...
TYPE *make_rec_type(FIELDLIST *fl)
{
...
STMTLIST  *make_stmts (STMT *stmt, STMTLIST *list)
{
    STMTLIST *s;
    if (stmt)   {
        s = anew (STMTLIST);
...
STMTIF *make_stmtif (EXPR *e, STMT *_then, STMT *_else)
{
    STMTIF *ifx = anew (STMTIF);
    ifx->if_expr = e;
    ifx->if_then = _then;
    ifx->if_else = _else;
    return ifx;
...
CASELIST *make_caselist (CASEARM *arm, CASELIST *re)
{
...
CASEARM  *make_casearm (EXPRLIST *es, STMT *st) 
{
...
STMTREPEAT *make_stmtrepeat (STMTLIST *st, EXPR *e)
{
    STMTREPEAT *rpt = anew (STMTREPEAT);
    rpt->rpt_stmts = st;
    rpt->rpt_expr = e;
    return rpt;
}

STMTFOR *make_stmtfor (IDENT *id, EXPR *ie, int di, EXPR *to, STMT *st)
{
    STMTFOR *forx = anew (STMTFOR);
    forx->for_name = make_binding(id);
    forx->for_init = ie;
    forx->for_to = to;
    forx->for_stmt = st;
    forx->for_upward = di;
    forx->for_limit_obj = 0;
...
STMT *make_compound_stmt (STMTLIST *list)
{
...
STMT *make_if_stmt (EXPR *e, STMT *_then, STMT *_else)
{
    STMT *s = make_stmt (StmtIf_);
    s->s.ifx = make_stmtif (e, _then, _else);
    return s;
}
...
STMT *make_repeat_stmt (STMTLIST *st, EXPR *e)
{
    STMT *s = make_stmt (StmtRepeat_);
    s->s.rpt = make_stmtrepeat(st, e);
    return s;
}

STMT *make_for_stmt (IDENT *id, EXPR *ie, int di, EXPR *to, STMT *st)
{
    STMT *s = make_stmt (StmtFor_);
    s->s.forx = make_stmtfor(id, ie, di, to, st);
    return s;
}
...
FIELD *make_fbinding(IDENT *id, TYPE *t)
{
    FIELD *f = anew (FIELD);
    f->field_id   = id;
    f->field_type = t;
...
FIELDLIST *make_bindflist(FIELD *f, FIELDLIST *ne) 
{
    FIELDLIST *fl = anew(FIELDLIST);
    if (!f->field_type)
        f->field_type = ne->this->field_type;
    fl->this = f;
    fl->rest = ne;
